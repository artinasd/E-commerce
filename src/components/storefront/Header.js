...
export default function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-white/95 backdrop-blur-xl">
    <div className="store-shell">
      <div className="flex min-h-[72px] items-center gap-2.5 sm:gap-3 lg:min-h-[76px] lg:gap-7">
        ...
        <div className="mr-auto flex items-center gap-1 lg:mr-0">
          <button type="button" onClick={() => setSearchOpen((v) => !v)} className="grid h-10 w-10 place-items-center text-slate-500 transition hover:bg-[var(--surface-soft)] hover:text-slate-950 md:hidden" aria-label="جستجو"><SearchIcon/></button>
          <Link href={user ? '/account' : '/login'} className="grid h-10 w-10 place-items-center text-slate-600 transition hover:bg-[var(--surface-soft)] hover:text-slate-950 sm:hidden" aria-label={user ? 'حساب کاربری' : 'ورود یا ثبت‌نام'}><UserIcon/></Link>
          {user ? <><Link href="/account" className="hidden h-10 items-center gap-2 px-2.5 text-[11px] font-extrabold transition hover:bg-[var(--surface-soft)] sm:flex"><span className="grid h-7 w-7 place-items-center rounded-full bg-[var(--brand-soft)] text-[var(--brand)]"><UserIcon/></span><span className="max-w-28 truncate">{user.name || user.email || 'حساب من'}</span></Link><button type="button" onClick={handleLogout} disabled={loggingOut} className="hidden px-2 text-[10px] font-bold text-slate-400 hover:text-red-600 disabled:opacity-50 sm:block">{loggingOut ? '...' : 'خروج'}</button></> : <Link href="/login" className="hidden h-10 items-center gap-2 border border-[var(--border)] px-3 text-[11px] font-extrabold hover:border-slate-300 sm:flex"><UserIcon/>ورود / ثبت‌نام</Link>}
          <Link href="/cart" className="grid h-10 w-10 place-items-center text-slate-600 transition hover:bg-[var(--surface-soft)] hover:text-slate-950" aria-label="سبد خرید"><CartIcon/></Link>
        </div>
      </div>
      ...
      <nav className="flex gap-1 overflow-x-auto border-t border-[var(--border)] py-2.5 lg:hidden" aria-label="ناوبری موبایل">{navItems.map(([label, href]) => <Link key={href} href={href} className={`shrink-0 px-3 py-1.5 text-[10px] font-extrabold ${pathname === href || pathname?.startsWith(`${href}/`) ? 'text-[var(--brand)]' : 'text-slate-500'}`}>{label}</Link>)}</nav>
    </div>
  </header>;
}
