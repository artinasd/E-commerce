...
export default function ProductVariantPricingManager({ productId, variants, onSaved }) {
  ...
  useEffect(() => {
    setDrafts(Object.fromEntries((variants || []).map((v) => [v.id, { sku: v.sku || '', name: v.name || '', price: v.price ?? '', compareAtPrice: v.compare_at_price ?? '', quantity: v.quantity ?? 0, lowStockThreshold: v.low_stock_threshold ?? 5 }])));
  }, [variants]);
  ...
  async function save(id) {
    ...
    try {
      const response = await fetch(`/api/admin/products/${productId}/variants/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify(drafts[id]) });
