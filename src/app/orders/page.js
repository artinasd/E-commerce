...
export default async function OrdersPage({ searchParams }) {
  ...
  if (!user) {
    return <div className="mx-auto max-w-2xl px-4 py-24 text-center"><h1 className="text-2xl font-black">برای مشاهده سفارش‌ها وارد شوید</h1><p className="mt-2 text-sm text-slate-500">سفارش‌های شما فقط برای حساب خودتان قابل مشاهده هستند.</p><Link href="/login?returnTo=/orders" className="mt-6 inline-flex rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">ورود به حساب</Link></div>;
  }
