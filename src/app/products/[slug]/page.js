    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)]">
      ...
      <section className="self-start lg:sticky lg:top-24">
        <div className="rounded-[28px] border border-[var(--border)] bg-white p-6 shadow-[0_18px_55px_rgba(23,23,23,.045)] sm:p-7">
          ...
          {product.short_description && <p className="mt-4 text-[12px] leading-7 text-slate-500">{product.short_description}</p>}
        ...
        {(product.description || product.category_name || product.brand_name) && <div className="mt-5 rounded-[28px] border border-[var(--border)] bg-white p-6 sm:p-7"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[var(--brand)]"/><h2 className="text-base font-black">معرفی محصول</h2></div>{product.description && <p className="mt-4 whitespace-pre-line text-[12px] leading-8 text-slate-600">{product.description}</p>}<dl className="mt-5 grid grid-cols-2 gap-3 text-[10px]"><div className="rounded-[14px] bg-[#fafaf8] p-3"><dt className="text-slate-400">دسته‌بندی</dt><dd className="mt-1 font-black text-slate-800">{product.category_name || '—'}</dd></div><div className="rounded-[14px] bg-[#fafaf8] p-3"><dt className="text-slate-400">برند</dt><dd className="mt-1 font-black text-slate-800">{product.brand_name || '—'}</dd></div></dl></div>}
      </section>
    </div>
