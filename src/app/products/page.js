  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <form className="flex flex-col gap-2 sm:flex-row" method="get">
          ...
          <select name="sort" defaultValue={params.sort || 'created_at'} className="h-11 rounded-xl border border-[var(--border)] bg-white px-3 text-sm outline-none">
            <option value="created_at">جدیدترین</option><option value="price">قیمت</option><option value="name">نام</option><option value="rating">امتیاز</option><option value="sales_count">پرفروش‌ترین</option>
          </select>
          <input type="hidden" name="direction" value={params.sort === 'price' ? 'asc' : 'desc'} />
      ...
      <div className="mt-8 flex flex-col gap-5 lg:flex-row">
        <aside className="w-full shrink-0 rounded-2xl border border-[var(--border)] bg-white p-4 lg:w-56">
          ...
          <div className="mt-5 border-t border-[var(--border)] pt-4">
            ...
            <div className="mt-2 grid gap-1 text-sm">
              ...
              <Link href={buildHref(params, { sort: 'rating', direction: 'desc', page: '' })} className="rounded-lg px-3 py-2 hover:bg-slate-50">بیشترین امتیاز</Link>
            </div>
          </div>
        </aside>
