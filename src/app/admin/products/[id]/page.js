...
function normalizeImages(images = []) {
  return Array.isArray(images) ? images.map((image) => ({
    id: Number(image.id), variantId: image.variantId ?? image.variant_id ?? null, url: image.url,
    altText: image.altText ?? image.alt_text ?? null, sortOrder: Number(image.sortOrder ?? image.sort_order ?? 0),
...
export default function AdminProductEditor({ params }) {
  ...
  useEffect(() => { Promise.resolve(params).then((p) => setId(p?.id ? String(p.id) : null)); }, [params]);

  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    Promise.all([
      fetch(`/api/admin/products/${id}/details`).then(async (r) => { const j = await r.json(); if (!r.ok) throw new Error(j?.error?.message || 'خطا در دریافت محصول'); return j; }),
      fetch('/api/admin/catalog/options').then(async (r) => { const j = await r.json(); if (!r.ok) throw new Error(j?.error?.message || 'خطا در دریافت گزینه‌ها'); return j; }),
    ]).then(([productResponse, optionsResponse]) => {
      if (cancelled) return;
      const productData = productResponse?.data ?? productResponse;
      const optionsData = optionsResponse?.data ?? optionsResponse;
      if (!productData?.product) throw new Error('اطلاعات محصول از سرور دریافت نشد.');
      setData({ product: productData.product, variants: Array.isArray(productData.variants) ? productData.variants : [], images: normalizeImages(productData.images) });
  ...
  if (!data?.product) return <section dir="rtl" className="p-8 text-slate-500">{message || 'در حال بارگذاری…'}</section>;
  const p = data.product;

  async function saveProduct(event) {
    event.preventDefault(); setSaving(true); setMessage('');
    try {
      const response = await fetch(`/api/admin/products/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ status: p.status, categoryId: p.category_id || null, brandId: p.brand_id || null, name: p.name, slug: p.slug, shortDescription: p.short_description, description: p.description, isFeatured: p.is_featured, seoTitle: p.seo_title, seoDescription: p.seo_description }) });
      const result = await response.json(); if (!response.ok) throw new Error(result?.error?.message || 'خطا در ذخیره محصول');
  ...
  async function addVariant(event) {
    ...
    try {
      ...
      const result = await response.json(); if (!response.ok) throw new Error(result?.error?.message || 'خطا در ایجاد تنوع');
      const createdVariant = result?.data?.variant ?? result?.variant;
      if (!createdVariant?.id) throw new Error('تنوع ایجاد شد اما شناسه آن دریافت نشد.');
      setData({ ...data, variants: [...data.variants, createdVariant] });
      setVariant({ sku: '', name: '', price: '', compareAtPrice: '', quantity: 0, lowStockThreshold: 5 }); setMessage('تنوع جدید ایجاد شد.');
    } catch (error) { setMessage(error.message); } finally { setSaving(false); }
  }

  return <section dir="rtl" className="space-y-6">
    <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-slate-500">کاتالوگ</p><h1 className="mt-1 text-3xl font-black">ویرایش محصول</h1></div><Link href="/admin/products" className="rounded-xl border bg-white px-4 py-2 text-sm font-bold">بازگشت</Link></div>
    <form onSubmit={saveProduct} className="grid gap-4 rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-bold">وضعیت<select value={p.status || 'DRAFT'} onChange={(e) => setData({ ...data, product: { ...p, status: e.target.value } })} className="rounded-xl border px-4 py-3 font-normal">{Object.entries(statuses).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
