...
export default async function AccountPage() {
  ...
  return <main dir="rtl" className="store-shell py-7 sm:py-10">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-2"><section className="border border-[var(--border)] bg-white p-5 sm:p-6"><div className="flex items-center justify-between"><h2 className="text-lg font-black">علاقه‌مندی‌ها</h2><Link href="/favorites" className="text-[10px] font-black text-slate-400">مشاهده همه ←</Link></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{favorites.map((f) => <Link href={`/products/${f.slug}`} key={f.product_id} className="group overflow-hidden bg-[#fafaf8] p-3"><div className="relative aspect-square overflow-hidden bg-white">{f.thumbnail_url ? <Image src={f.thumbnail_url} alt={f.name} fill sizes="(min-width: 640px) 120px, 42vw" className="object-contain transition duration-300 group-hover:scale-[1.03]"/> : <span className="flex h-full items-center justify-center text-[9px] text-slate-400">تصویر ندارد</span>}</div><p className="mt-2 line-clamp-2 text-[9px] font-black leading-5">{f.name}</p></Link>)}{!favorites.length && <p className="col-span-full bg-[#fafaf8] p-6 text-center text-[10px] text-slate-400">هنوز محصولی به علاقه‌مندی‌ها اضافه نکرده‌اید.</p>}</div></section><section className="border border-[var(--border)] bg-white p-5 sm:p-6"><div className="flex items-center justify-between"><h2 className="text-lg font-black">آدرس‌های من</h2><Link href="/account/addresses" className="text-[10px] font-black text-slate-400">مدیریت آدرس‌ها ←</Link></div><div className="mt-5 grid gap-3">{addresses.slice(0,3).map((a) => <div key={a.id} className="bg-[#fafaf8] p-4 text-[10px]"><div className="flex justify-between gap-3"><b>{a.recipient_name}</b>{a.is_default ? <span className="font-black text-emerald-600">پیش‌فرض</span> : null}</div><p className="mt-2 leading-6 text-slate-500">{a.province}، {a.city}<br/>{a.address_line}</p></div>)}{!addresses.length && <p className="bg-[#fafaf8] p-6 text-center text-[10px] text-slate-400">هنوز آدرسی ثبت نکرده‌اید.</p>}</div></section></div>
  </main>;
}
function Stat({ value, label }) { return <div className="border border-[var(--border)] bg-white p-5 shadow-[0_8px_24px_rgba(23,23,23,.02)]"><p className="text-2xl font-black">{Number(value).toLocaleString('fa-IR')}</p><p className="mt-1 text-[9px] font-bold text-slate-400">{label}</p></div>; }
function Info({ label, value }) { return <div className="bg-[#fafaf8] p-4"><span className="text-slate-400">{label}</span><p className="mt-1 font-black text-slate-800">{value}</p></div>; }
