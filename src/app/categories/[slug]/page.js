...
import { getCategoryBySlug, getProducts } from '../../../server/catalog/service.js';
...
export async function generateMetadata({ params }) {
  ...
  return { title: category ? `${category.name} | فروشگاه` : 'دسته‌بندی' };
}
...
export default async function CategoryPage({ params, searchParams }) {
  ...
  const hrefForPage = (nextPage) => `/categories/${encodeURIComponent(slug)}?${new URLSearchParams([...paramsForPage, ['page', String(nextPage)]]).toString()}`;
  ...
  return (
    <main dir="rtl" className="store-shell py-7 sm:py-10">
      <nav className="flex items-center gap-2 text-xs font-bold text-slate-400">
        ...
        <span className="font-black text-slate-700">{category.name}</span>
      </nav>
      ...
      <header className="mt-6 overflow-hidden border border-[var(--border)] bg-white">
        <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:p-8">
          {category.image_url ? <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden bg-slate-50 sm:h-28 sm:w-28"><img src={category.image_url} alt={category.name} className="h-full w-full object-cover" /></div> : null}
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[.18em] text-[var(--brand)]">دسته‌بندی محصولات</p>
            <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">{category.name}</h1>
            <p className="mt-2 text-sm leading-7 text-slate-500">محصولات این دسته‌بندی را بررسی کنید و کالای موردنظرتان را پیدا کنید.</p>
          </div>
        </div>
      </header>
      ...
      <div className="mt-7 flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] pb-4">
        ...
        <div className="flex gap-2">
          <Link href={`/categories/${encodeURIComponent(slug)}`} className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-xs font-black hover:border-slate-300">جدیدترین</Link>
          <Link href={`/categories/${encodeURIComponent(slug)}?sort=price&direction=asc`} className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-xs font-black hover:border-slate-300">ارزان‌ترین</Link>
          <Link href={`/categories/${encodeURIComponent(slug)}?sort=price&direction=desc`} className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-xs font-black hover:border-slate-300">گران‌ترین</Link>
        </div>
      </div>

      {products.length ? <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="mt-6 border border-[var(--border)] bg-white px-6 py-20 text-center"><h2 className="text-xl font-black">محصولی در این دسته‌بندی پیدا نشد</h2><p className="mt-2 text-sm text-slate-500">فعلاً محصول فعالی برای نمایش وجود ندارد.</p><Link href="/products" className="mt-6 inline-flex bg-slate-950 px-6 py-3 text-sm font-black text-white hover:bg-[var(--brand)]">مشاهده همه محصولات</Link></div>}
