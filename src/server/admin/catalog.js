...
export async function updateAdminBrandLogo(brandId, logoUrl) {
  ...
  if (logo && logo.length > 1000) throw new Error('آدرس تصویر بیش از حد طولانی است.');
  const result = await query('UPDATE brands SET logo_url = ? WHERE id = ? AND deleted_at IS NULL', [logo, id]);
  if (!result.affectedRows) throw new Error('برند پیدا نشد.');
  return { id, logo_url: logo };
}
...
export async function updateAdminCategoryImage(categoryId, imageUrl) {
  ...
  if (image && image.length > 1000) throw new Error('آدرس تصویر بیش از حد طولانی است.');
  const result = await query('UPDATE categories SET image_url = ? WHERE id = ? AND deleted_at IS NULL', [image, id]);
  if (!result.affectedRows) throw new Error('دسته‌بندی پیدا نشد.');
  return { id, image_url: image };
}

export async function getAdminProduct(productId) {
  await admin();
  const [products, variants, images] = await Promise.all([
    query(`SELECT p.id, p.category_id, p.brand_id, p.name, p.slug, p.short_description, p.description, p.status, p.is_featured, p.meta_title AS seo_title, p.meta_description AS seo_description FROM products p WHERE p.id = ? AND p.deleted_at IS NULL LIMIT 1`, [productId]),
    query(`SELECT v.id, v.sku, v.name, v.price, v.compare_at_price, v.is_active, COALESCE(i.quantity, 0) AS quantity, COALESCE(i.reserved_quantity, 0) AS reserved_quantity, COALESCE(i.low_stock_threshold, 5) AS low_stock_threshold FROM product_variants v LEFT JOIN inventory i ON i.variant_id = v.id WHERE v.product_id = ? ORDER BY v.id ASC`, [productId]),
