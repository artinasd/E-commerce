...
export async function getAdminDashboard() {
  ...
  const [sales, orders, customers, inventory, products, recentOrders, topProducts] = await Promise.all([
    ...
    query(`SELECT COUNT(*) AS variants, COALESCE(SUM(GREATEST(i.quantity-i.reserved_quantity,0)),0) AS available_units, COALESCE(SUM(CASE WHEN i.quantity-i.reserved_quantity <= i.low_stock_threshold THEN 1 ELSE 0 END),0) AS low_stock FROM inventory i INNER JOIN product_variants v ON v.id=i.variant_id INNER JOIN products p ON p.id=v.product_id WHERE p.deleted_at IS NULL AND p.status='ACTIVE' AND v.is_active=TRUE`),
    query(`SELECT COUNT(*) AS active_products FROM products WHERE deleted_at IS NULL AND status='ACTIVE'`),
  ...
  return { sales: sales[0], orders, customers: customers[0], inventory: inventory[0], products: products[0], recentOrders, topProducts };
}
