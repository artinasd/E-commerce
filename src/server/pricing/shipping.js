...
export async function listAvailableShippingMethods(connection, province) {
  if (typeof province !== 'string' || !province.trim()) throw new Error('Delivery province is required.');
  const [rows] = await connection.execute(`SELECT id, name, code, description, base_amount, free_shipping_minimum FROM shipping_methods WHERE is_active = TRUE AND deleted_at IS NULL AND (NOT EXISTS (SELECT 1 FROM shipping_method_provinces smp WHERE smp.shipping_method_id = shipping_methods.id) OR EXISTS (SELECT 1 FROM shipping_method_provinces smp WHERE smp.shipping_method_id = shipping_methods.id AND smp.province = ?)) ORDER BY sort_order ASC, id ASC`, [province.trim()]);
  return rows;
}
