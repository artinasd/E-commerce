...
function mapReview(review) {
  ...
  return {
    ...
    authorName: `${first} ${last}`.trim() || 'کاربر فروشگاه',
...
export async function getProductReviews(productId, options = {}) {
  const [summary, reviews] = await Promise.all([
    ...
    listApprovedReviewsForProduct(productId, options),
  ]);
...
export async function submitProductReview(userId, productId, body) {
  ...
  if (!eligibleItem) {
    const error = new Error('Only customers who purchased and received this product can review it.');
    error.code = 'REVIEW_PURCHASE_REQUIRED';
    throw error;
  }

  const existing = await findReviewByOrderItemForUser(input.orderItemId, userId);
  if (existing) {
    const error = new Error('You have already reviewed this purchased item.');
