...
export async function reserveInventory(connection, variantId, quantity) {
  ...
  if (result.affectedRows !== 1) {
    throw new Error('Insufficient inventory or inventory record not found.');
  }
}
...
export async function releaseInventory(connection, variantId, quantity) {
  ...
  if (result.affectedRows !== 1) {
    throw new Error('Unable to release inventory reservation.');
  }
}

export async function decrementInventory(connection, variantId, quantity) {
  if (!Number.isInteger(quantity) || quantity < 1) {
    throw new Error('Inventory decrement quantity must be a positive integer.');
  }
  ...
  if (result.affectedRows !== 1) {
    throw new Error('Unable to decrement inventory.');
  }
}
...
export async function reserveMultipleInventory(items) {
  return withTransaction(async (connection) => {
    for (const item of items) {
      await reserveInventory(connection, item.variantId, item.quantity);
    }
  });
}
