  ...
  return query(
    `SELECT
     ...
     LEFT JOIN product_images pi
       ON pi.product_id = p.id AND pi.variant_id = v.id AND pi.is_primary = TRUE
     WHERE ci.cart_id = ?
...
export async function addCartItem(userId, variantId, quantity) {
  ...
  return withTransaction(async (connection) => {
    ...
    if (cartRows.length === 0) {
      await connection.execute(`INSERT INTO carts (user_id) VALUES (?)`, [userId]);
    }

    const [resolvedCartRows] = await connection.execute(
      `SELECT id FROM carts WHERE user_id = ? LIMIT 1 FOR UPDATE`,
      [userId],
    );
    ...
    const [variantRows] = await connection.execute(
      `SELECT v.id, GREATEST(COALESCE(i.quantity, 0) - COALESCE(i.reserved_quantity, 0), 0) AS available_quantity
    ...
    const [existingRows] = await connection.execute(
      `SELECT id, quantity FROM cart_items WHERE cart_id = ? AND variant_id = ? FOR UPDATE`,
      [cartId, variantId],
    );

    const nextQuantity = (existingRows[0]?.quantity ?? 0) + quantity;
    if (nextQuantity > Number(variant.available_quantity)) {
    ...
    if (existingRows[0]) {
      await connection.execute(
        ...
        [nextQuantity, existingRows[0].id],
      );
    } else {
      await connection.execute(
...
export async function updateCartItem(userId, itemId, quantity) {
  ...
  await query(
    `UPDATE cart_items ci
     ...
     WHERE ci.id = ?
       AND c.user_id = ?
       AND GREATEST(COALESCE(i.quantity, 0) - COALESCE(i.reserved_quantity, 0), 0) >= ?`,
    [quantity, itemId, userId, quantity],
  );

  const updated = await query(
    `SELECT ci.id FROM cart_items ci INNER JOIN carts c ON c.id = ci.cart_id
      WHERE ci.id = ? AND c.user_id = ? LIMIT 1`,
    [itemId, userId],
  );

  if (!updated[0]) throw new Error('Requested quantity exceeds available inventory.');
}
